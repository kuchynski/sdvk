...
CTable::CTable(QWidget *parent) : QWidget(parent), first_line(0), end_line(0), kol_line_screen(0),
    visiable_title(false), visiable_buttons(false), style_back(esColor), number_checked_line(-1)
{
    QWidget::setFocusPolicy(Qt::ClickFocus);//Qt::StrongFocus);
    for(int i = 0; i < 8; i ++)
    {
        SButton b;
        b.state = esFree;
...
void CTable::AddLine(const QList<QString> &sl, const bool paint)
{
    ...
    if(lines.size() == 1)
    {
        number_checked_line = 0;
        lines[lines.size()- 1].state = esChecked;
    }
    else
    {
...
QList<QString> CTable::GetLineText(const unsigned &n)
{
    QList<QString> ret;
    if(n < lines.size())
        for(unsigned i = 0; i < lines[n].field.size(); i ++)
            ret << lines[n].field[i];
    return ret;
}
QString CTable::GetPoleText(const unsigned &l, const unsigned &n)
{
    QString ret = "";
    if(l < lines.size())
        if(n < lines[l].field.size())
            ret = lines[l].field[n];
    return ret;
}
void CTable::SubLine(const unsigned &n, const bool paint)
{
    if(n >= lines.size())
        return;
    bool recheked = (lines[n].state == esChecked);
    lines.removeAt(n);
    if(recheked)
        if(n < lines.size())
        ...
        {
            lines[n-1].state = esChecked;
            number_checked_line = n-1;
        }
    if(paint)
        ReSize();
}
...
void CTable::ReSize()
{
    const unsigned width_pic = width();
    const unsigned hieght_pic = height();
    const unsigned hieght_line = init_struct.hieght_line;
...
void CTable::mousePressEvent(QMouseEvent *pe)
{
  ...
  //  releaseKeyboard();
    ...
    if(visiable_title && title.IsMy(x, y) )
        return;
    int new_checked = -1;
    for(int i = first_line; i < end_line; i ++)
//    for(int i = 0; i < lines.size(); i ++)
...
{
    for(int i = first_line; i < end_line; i ++)
        if(lines[i].IsMy(pe->x(), pe->y()))
        {
            emit ClickDoubleLine(i);
            break;
        }
}
...
void CTable::SetImageBack(const QImage &ib, const QRect &r)
{
