...
CChecks::CChecks(QWidget *parent) : QWidget(parent), focus(false), style_back(esColor)
{
...
void CChecks::Paint()
{
    ...
    int stap_gr[5] = {255, 160, 200, 255, 200};
...
//        QRect rect_check(buttons[i].x1+2, buttons[i].y1+2, buttons[i].x2 - buttons[i].x1-2, buttons[i].y2 - buttons[i].y1-2);
        QRect rect_check(buttons[i].x1, buttons[i].y1, buttons[i].x2 - buttons[i].x1, buttons[i].y2 - buttons[i].y1);
        rect_no_check = rect_check;
        int sglaz = (buttons[i].y2 - buttons[i].y1) / init_struct.sglaz_const;
...
void CChecks::SetFocusAll(const bool &state)
{
    for(unsigned n = 0; n < buttons.size(); n ++)
        buttons[n].state = (state)? esFocus : esFree;
}
...
bool CChecks::GetChecked(const unsigned &n)const
{
    return (n < buttons.size())? (buttons[n].state == esChecked) : false;
}
...
void CChecks::SetImageBack(const QImage &ib, const QRect &r)
{
